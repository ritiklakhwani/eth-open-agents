...
- **Codebase name:** `tama` — short, lowercase, what shows up in folders, package names, ENS root (`tama.eth`), DB filenames
- **Product/brand name:** `PetCity` — what users + judges see in dashboard, video, Devfolio submission
- **Single repo:** ONE GitHub repo at `<your-username>/tama`. Private during Days 1-4 (prevents copycat), flipped to public on Day 5 morning before Devfolio submission (judges need to browse the code). No sidecar repos, no separate infra/showcase repo — everything (contracts, hub, web, pet-runtime, packages, scripts) lives in this single monorepo.

## Team work division — Ritik (UI + integrations) and Karmanay (pet backend)

Two-person team. Ritik has $200 Claude budget + wants UI work. Karmanay has $20 Claude budget. Division by **phases** (mapping to BUILD.md), not days. Ritik takes ~2x the load (51h vs 24h) reflecting budget ratio.

**Principle:** Ritik owns the visible product (frontend + contracts + KeeperHub) and outward-facing integrations. Karmanay owns the invisible "pet life support system" (AXL → Hub spawning → worker running → 0G storing). Each phase has one owner. Zero file overlap.

### Phase ownership matrix

| Phase | Owner | Hours | Folders touched |
|---|---|---|---|
| 1 — Scaffold | Ritik (one-time) | 1h | root, all initial folders |
| 2 — Contracts | **Ritik** | 5h | `contracts/**`, `packages/contracts-sdk/**` |
| 3 — ENS setup | **Ritik** | 2h | `packages/ens/**` |
| 4 — AXL plumbing | **Karmanay** | 4h | `apps/hub/src/axl-config.ts`, `packages/pet-runtime/src/axl.ts` |
| 5 — Hub + PetSupervisor | **Karmanay** | 8h | `apps/hub/**` |
| 6 — Pet Runtime | **Karmanay** | 8h | `packages/pet-runtime/**` |
| 7a — 0G Storage SDK | **Karmanay** | 4h | `packages/og-storage/**` |
| 7b — Sprite gen API | **Ritik** | 3h | `packages/sprite-gen/**`, `apps/web/src/app/api/pets/sprite/**` |
| 8 — KeeperHub workflows | **Ritik** | 8h | `packages/keeperhub/**` |
| 9 — Frontend (FULL) | **Ritik** | 24h | `apps/web/**` (entire frontend) |
| 10 — Demo + submission | Both | 6h R / 4h K | demo fixtures, video, READMEs |

**Totals: Ritik ~51h, Karmanay ~24h.**

### Folder ownership matrix (zero overlap)

**Ritik owns:**
- `contracts/**`
- `packages/ens/**`, `packages/sprite-gen/**`, `packages/contracts-sdk/**`, `packages/keeperhub/**`
- `packages/shared-types/**` (writes; Karmanay imports as read-only)
- `apps/web/**` (entire frontend including page, components, phaser, public assets, Tiled map)
- `apps/web/src/app/api/pets/sprite/**`, `apps/web/src/app/api/pets/blob/**`, `apps/web/src/app/api/demo/**`
- `scripts/**`, `bin/**`, `.env*`, root config files, `apps/web/package.json` (frontend deps)

**Karmanay owns:**
- `apps/hub/**` (entire Hub: Fastify, supervisor, socket.io server, db, axl-config gen)
- `packages/pet-runtime/**` (worker, Brain, Memory, Blob, AXL wrapper, activities)
- `packages/og-storage/**` (0G SDK wrapper)
- `apps/web/src/app/api/sse/**` (SSE for pet events from Hub)
- `apps/hub/package.json`, `packages/pet-runtime/package.json` (his own deps)

**Hard rules:**
- Karmanay never touches: `contracts/`, `packages/ens/`, `packages/sprite-gen/`, `packages/keeperhub/`, `packages/contracts-sdk/`, `apps/web/src/components/`, `apps/web/public/`, `scripts/`, root config
- Ritik never touches: `apps/hub/`, `packages/pet-runtime/`, `packages/og-storage/`

### Phase-by-phase ordering (who unblocks who)

```
DAY 1
  Ritik:   Phase 1 (scaffold) → Phase 2 (contracts deploy to Sepolia) → Phase 3 (ENS)
  Karmanay: Phase 4 (AXL spike — 2 nodes talking on localhost — CRITICAL by EOD)

DAY 2
  Ritik:   Phase 7b (sprite gen API) + start Phase 9 (Phaser scaffold + Tiled v1 map)
  Karmanay: Phase 5 (Hub + supervisor + socket.io server)
  ↓ Karmanay's socket.io server READY for Ritik's multiplayer client

DAY 3
  Ritik:   Phase 8 (KeeperHub workflows) + Phase 9 (more frontend, Adoption flow, SpriteUploader)
  Karmanay: Phase 6 (Pet Runtime worker) + Phase 7a (0G wrapper)
  ↓ Karmanay's pet workers READY — end-to-end testing begins

DAY 4
  Ritik:   Phase 9 polish (Dashboard, BattleArena, SubscriptionPanel, MailboxFlow)
  Karmanay: Bug fixes in Hub/Runtime, integration testing support, edge cases

DAY 5
  Both: Phase 10 — rehearsal, video, READMEs, Devfolio submit
```

### How Ritik and Karmanay integrate (API contract)

The only "shared" file is `packages/shared-types/index.ts` (Ritik writes, Karmanay imports).

**What Karmanay's Hub exposes to Ritik's frontend:**
- WebSocket on `:3001`: events `positions`, `chat`, `zoneEnter`
- REST: `GET /api/pets`, `GET /api/sse/:petId`

**What Karmanay's Hub imports from Ritik's packages:**
- `packages/keeperhub` — `createMailboxWorkflow()`, `createBattleEscrowRelease()`, `createRecurringAllowance()`, etc.
- `packages/contracts-sdk` — viem ABIs for Mint event watching
- `packages/ens` — `readPeerIdFromENS()`, `mintPetSubname()`

**What Ritik's frontend imports from Karmanay's code:**
- Nothing directly. Frontend talks to Hub via socket + SSE only.

### Branch + PR workflow (zero merge conflicts)

```
main                       # protected, only PR merges
ritik/contracts            # Phase 2
ritik/ens                  # Phase 3
ritik/sprite-gen           # Phase 7b
ritik/keeperhub            # Phase 8
ritik/frontend             # Phase 9
ritik/demo                 # Phase 10
karman/axl                 # Phase 4
karman/hub                 # Phase 5
...
- Both review each other's PRs (10 min reviews)
- Merge to main at EOD daily to stay in sync
- `pnpm install` from root after every pull
- New deps in apps/hub or packages/pet-runtime → Karmanay installs himself
- New deps in apps/web or contracts → Ritik installs

### Daily sync

- 15-min morning standup (yesterday / today / blockers)
- 15-min EOD sync (demo what works, push branches, merge to main)
- Discord for blockers (anything blocking the other → ping immediately)
...
## The World — Gather.town-style multiplayer pixel world

A top-down 2D world built with **Phaser 3** + **Tiled** (map editor). Multiple zones in one shared world. All pets visible to all users (multiplayer via WebSocket). User drives their pet with arrow keys; AI controls dialogue/memory/personality.

**Zones (rectangles defined in Tiled, trigger on player entry):**

| Zone | What happens |
|---|---|
| **Park / Garden** | Pets meet, ambient social, AI-driven chat when adjacent |
| **Office desks** | Pet on desk → triggers Subscription Pet workflow scan |
| **Battle Arena** | Pet enters ring → joins battle queue, KeeperHub stake escrow |
| **Lounge / Private area** | 1:1 owner-pet LLM chat modal opens |
| **Kitchen / Café** | Care stats restore (energy/hunger refill) |
| **Mailbox / Post Office** | KeeperHub conditional fires when pet enters; queued gifts deliver |

**Movement modes:**
- Manual (arrow keys): user drives their pet — intentional, demo-able
- Autonomous (when idle): pet wanders via simple AI

The AI personality stays — arrow keys move the body, the LLM still drives dialogue + memory + decisions. Walking your pet adjacent to another pet auto-triggers AXL→LLM chat. Best of both worlds.
...
| Layer | Pick |
|---|---|
| Frontend | Next.js 15 + Tailwind + shadcn + **Phaser 3** (game scene) + **Tiled** (map editor) + **socket.io-client** (multiplayer) + RainbowKit + viem + wagmi |
| Contracts | Foundry + solady ERC-721 |
| Pet runtime | Node + tsx, one OS process per pet, child_process.fork from Hub |
| Hub server | Node + Fastify + **socket.io** (multiplayer position broadcast) |
